...
exports.insert = function(uaccount, upassword, uname, uinfo, res) {
	pool.getConnection(function(err, connection) {
		...
		var tasks = [function(callback) {
			...
			connection.query('INSERT INTO sys_user (u_name,u_info) values (?,?)', 
				[uname, uinfo], 
				function(err, result) {
				callback(err, result.insertId); // 生成的ID会传给下一个任务
			})
		}, function(insertId, callback) {
			connection.query('INSERT INTO sys_passport (u_account,u_password,passport_id) values (?,?,?)',
				[uaccount,upassword,insertId], 
				function(err, result) {
				callback(err);
			});
		}, function(callback) {
...
exports.login = function(uaccount, upassword, res) {
	...
	pool.getConnection(function(err, connection) {
			...
			connection.query('SELECT * FROM sys_passport WHERE u_account = ? AND u_password = ?',
			 [uaccount,upassword], 
			 function(err, result) {
...
exports.update = function(passportId,oldPwd,newPwd,res){
	pool.getConnection(function(err,connection){
		connection.query('UPDATE sys_passport SET u_password = ? WHERE u_password = ? AND passport_id = ?',
			[newPwd,oldPwd,passportId],
			function(err,result){
